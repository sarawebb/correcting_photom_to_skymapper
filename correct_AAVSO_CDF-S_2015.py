...
input_cats = '/fred/oz100/NOAO_archive/archive_NOAO_data/data_outputs/'+year+'/'+month+'/'+field+'/vizier_cats/AAVOS.ascii'
print(input_cats)
...
for i in path_list: 
	...
	for filename in os.listdir(i):
		...
		if filename.endswith('.cat'):
			...
			ra, dec, g_mag, g_mag_err = np.loadtxt(input_cats, unpack = True, skiprows =1, usecols= (0,1,2,3))
			SM_X = np.empty((len(ra), 5), dtype=np.float64)
			SM_X[:, 0] = ra
			SM_X[:, 1] = dec
			SM_X[:, 2] = g_mag
			...
			photom_correction_path_images = photom_correction_path + 'AAVOS_checkphotom_plots/'
			...
			if len(match) != 0: 
				...
				if len(diff_mags) > 0: 
					...
					plt.savefig(photom_correction_path_images + filename + 'AAVOS_uncorrection.png', overwrite = True)
					#plt.show()
					plt.close()
					...
					plt.savefig(photom_correction_path_images + filename + 'AAVOS_corrected.png', overwrite = True)
					plt.close()
					...
					t = new_cat
					output= final_source_cat_path + '/'+ filename + 'AAVOS_CORRECTED.ascii'
					#print(output)
					t.write(output, format= 'ascii' , overwrite = True)
				else: 
					print('No gband from AAVOS for this ccd')
					if len(av_correction) != 0:
						mean_of_means = np.mean(av_correction)
						print('mean_of_means: ' + str(mean_of_means))
					
						if mean_of_means == 0: 
							print('No matching AAVOS sources, NOT correcting for this ccd')
							...
							t = new_cat
							output= final_source_cat_path + '/' + filename + 'AAVOS_NOT_CORRECTED.ascii'
							t.write(output, format= 'ascii' , overwrite = True)
	...
	try: 		
		...
		output2 = photom_correction_path + '/average_correction_per_ccd_AAVOS.ascii'
	...
	try: 	
		...
		output3 = photom_correction_path + '/odd_values_found_per_ccd_AAVOS.ascii'
