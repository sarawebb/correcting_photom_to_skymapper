...
for i in path_list: 
	for filename in os.listdir(i):
		if filename.endswith('.cat'):
			...
			DWF_X = np.empty((len(MAG_AUTO), 2) ,dtype =  np.float64)
			DWF_X[:,0] = ALPHA_J2000
			DWF_X[:,1] = DELTA_J2000
			
			RA, DEC, Gmag_vega, Gmag_vega_err, Gmag_AB = np.loadtxt(output_cats + 'gaiaDR2.ascii', unpack = True, skiprows=1)
			
			GAIADR2_cat = Table()
			GAIADR2_cat['RA'] = RA
			GAIADR2_cat['DEC'] = DEC
			GAIADR2_cat['g_mag_AB'] = Gmag_AB
			GAIADR2_cat['g_mag_vega'] = Gmag_vega
			GAIADR2_cat['g_mag_vega_err'] = Gmag_vega_err
			
			GAIADR2_X = np.empty((len(RA), 2), dtype =  np.float64)
			GAIADR2_X[:,0] = RA
			GAIADR2_X[:,1] = DEC
			...
			dist_between, ind_row = crossmatch_angular(DWF_X, GAIADR2_X, max_radius)
			match = ~np.isinf(dist_between)
			
			match_table_gaiaDR2 = Table()
			match_table_gaiaDR2['matched_true_false'] = match 
			match_table_gaiaDR2['matched_ID'] = ind_row
			match_table_gaiaDR2['matched_DWF_data_gmag'] =  ccd_SE_cat['mag_auto']
			match_table_gaiaDR2['matched_DWF_data_gmag_err'] = ccd_SE_cat['mag_auto_err']
			match_table_gaiaDR2['matched_DWF_RA'] = ccd_SE_cat['RA'] 
			match_table_gaiaDR2['matched_DWF_DEC'] = ccd_SE_cat['DEC'] 
			
			gaia_match_true =[]
			gaia_row_matched = []
			dwf_gmag_matched = []
			dwf_gmag_error_matched = []
			dwf_ra_matched = []
			dwf_dwf_matched = []
			
			for row in match_table_gaiaDR2:
				if row['matched_true_false'] == True: 
					
			
			print(match_table_gaiaDR2)	
