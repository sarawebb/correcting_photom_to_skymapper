...
input_cats = '/fred/oz100/NOAO_archive/archive_NOAO_data/scripts/correct_photom/skymapper/shortlisted_stars_for_photom/'+field+'_SM.csv_SHORTLISTED.ascii'
...
for i in path_list: 
	for filename in os.listdir(i):
		...
		if filename.endswith('.cat'):
			...
			final_source_cat_path = str(filename_directory) + '/final_source_cats/'
			photom_correction_path_images = str(photom_correction_path) + '/skymapper_checkphotom_plots/'
			...
			#print('photom_correction_path ' + str(photom_correction_path))
			#print('final_source_cat_path ' + str(final_source_cat_path))
			#print('photom_correction_path_images ' + str(photom_correction_path_images))
			...
			try: 
				...
				plt.savefig(photom_correction_path_images + filename + 'skymapper_uncorrection.png')
				plt.close()
				...
				plt.savefig(photom_correction_path_images + filename + 'skymapper_corrected.png')
				...
				source_match_table['DWF_g_mag_ZPoffset'] = source_match_table['DWF_gmag'] - mean_of_means
				new_cat = Table() 
				new_cat['RA'] = ccd_SE_cat['RA']
				new_cat['DEC'] = ccd_SE_cat['DEC']
				new_cat['g_mag'] = corrected_g_mags
